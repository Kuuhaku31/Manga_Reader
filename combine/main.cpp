...
string root_path   = "D:\\Project\\FishTank\\Manga_Reader\\test\\img\\01";
string output_path = "D:\\Project\\FishTank\\Manga_Reader\\test\\img\\output";
...
// 判断一张图片的情况
bool
isSingle(int count, string pathA, string pathB)
{
    // 一张图片的情况
    vector<int> single = { 0 };
...
Set_couple()
{
    for(int i = 0, count = 0; i < files.size(); count++)
    {
        ...
        string imageB_path = files[i + 1];

        // 如果判断为一组
        if(isSingle(count, imageA_path, imageB_path))
        {
            cout << "将" << files[i] << "判断为一组" << endl;
            cop.imgA_path = files[i];
